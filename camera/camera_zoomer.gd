...
@export var _zoom_in_ratio: float = 2
@export var _zoom_out_ratio: float = 0.5
@export var _zoom_step: Vector2 = Vector2(0.1, 0.1)
@export var _zoom_speed: float = 20
...
func _process(delta):
	...
	if speed_up:
		camera.zoom = camera.zoom.lerp(_target_zoom, _zoom_speed * delta * _speed_up_multiplier)
	else:
			camera.zoom = camera.zoom.lerp(_target_zoom, _zoom_speed * delta)	
